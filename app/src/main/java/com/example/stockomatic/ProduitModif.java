...
public class ProduitModif extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_produit_modif);
        ...
        Log.i("doInBack","Test lIdCateg modification produit : " + lIdCateg);
        // recuperation des id des boutons
        Button btnModifProd = this.findViewById(R.id.btnModifProduit);
        Button btnCancelProd = this.findViewById(R.id.btnCancelProduit);
        ImageButton btnScanIdProd = this.findViewById(R.id.btnScanIdProdModifProduit);
        // recuperation des id des EditText
        EditText edIdProd = (EditText)findViewById(R.id.txtModifIdProd);
        EditText edLibelleProd = (EditText)findViewById(R.id.txtModifLibelleProd);
        EditText edPrixProd = (EditText)findViewById(R.id.txtModifPrixProd);
        EditText edDescriptionProd = (EditText)findViewById(R.id.txtModifDescriptionProd);
        // recuperation des id des spinner
        Spinner spinLibelleCateg = this.findViewById(R.id.spinLibelleCategModifProduit);
        ...
        // recup de l'id
        String lIdCategSelect = sgbd.getIdCategProduit(leLibelle);
        // recup de la position
        int position = lesCategorieViaSQL.indexOf(lIdCategSelect);
        // selection du spinner
        spinLibelleCateg.setSelection(position);
        // ajout des valeurs dans les EditText
        edIdProd.setText(lIdProd);
        edLibelleProd.setText(leLibelle);
        edPrixProd.setText(lePrix.toString());
        edDescriptionProd.setText(laDescription);
        // mise en place de setOnClickListener pour pouvoir faire une action en cliquant sur les boutons
        btnModifProd.setOnClickListener(this);
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.btnModifProduit){
            // recuperation de la page a afficher
            Intent pageProduiModif = new Intent(this, VueProduit.class);
            // recuperation des id des editText
            EditText edIdProd = (EditText)findViewById(R.id.txtModifIdProd);
            EditText edLibelleProd = (EditText)findViewById(R.id.txtModifLibelleProd);
            EditText edPrixProd = (EditText)findViewById(R.id.txtModifPrixProd);
            ...
            Spinner spinLibelleCateg = (Spinner)findViewById(R.id.spinLibelleCategModifProduit);
            // instanciation du bundle
            Bundle extras = getIntent().getExtras();
            // recuperation des valeurs via le bundle
            String lIdProduitOriginal = extras.getString("lId");
            // test dans le logcat pour verifier que les valeurs sont bien récupérer
            Log.i("doInBack","Test lIdProduitOriginal modification produit : " + lIdProduitOriginal);
            ...
            int leLibelleCategSelect = spinLibelleCateg.getSelectedItemPosition();
            // recuperation du libelle de la categorie
            String leLibelleCateg = lesCategorieViaSQL.get(leLibelleCategSelect);
            // recup de l'id de la categorie
            String lIdCategListe = sgbd.getidCategCategorie(leLibelleCateg);
            // test dans le logcat pour verifier que les valeurs sont bien récupérer
            Log.i("doInBack","Test lIdProduit modification produit : " + lIdProduit);
            Log.i("doInBack","Test leLibelle modification produit : " + leLibelle);
            Log.i("doInBack","Test lePrix modification produit : " + lePrix);
            Log.i("doInBack","Test laDescription modification produit : " + laDescription);
            Log.i("doInBack","Test lIdCateg modification produit : " + lIdCategListe);
            // instanciation du produit
            Produits leProduit = new Produits(lIdProduit,leLibelle,lePrix,laDescription,lIdCategListe);
            // methode de modification des données dans le sgbd
            sgbd.ModifProduit(leProduit,lIdProduitOriginal);
            // fermeture de la sgbd
            sgbd.close();
            // affichage de la page
            startActivity(pageProduiModif);
            // fermeture de la page en cours
            this.finish();
        ...
        if (id == R.id.btnCancelProduit){
            ...
            this.finish();
            // instanciation du toast
            Toast toast = Toast.makeText(getApplicationContext(),"Abandon de la modification du produit", Toast.LENGTH_SHORT);
            // affichage d'un toast
            toast.show();
        }
        // si le bouton cliqué est scan
        if (id == R.id.btnScanIdProdModifProduit){
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (result != null){
            if (result.getContents() == null){
                ...
                EditText txtIdCatef = (EditText) findViewById(R.id.txtModifIdProd);
